...
def encrypt_vector_to_ciphertext(indices): 
    count = 0
    ciphertexts = []
...
def encrypt_gradient_for_aggregation(local_list,indexes):
    ...
    selected_gradients_list = selected_gradients.tolist()

    start_time = time.perf_counter()  
    ciphertexts = []
    count = 0
    for i in range(0, len(indexes), max_slots):
        chunk = selected_gradients_list[i:i+max_slots]
        pt = cc.MakeCKKSPackedPlaintext(chunk)
        ct = cc.Encrypt(keys.publicKey, pt)
        ciphertexts.append(ct)
        count=count+1
...
for round_idx in tqdm(range(fed_args.num_rounds)):
    ...
    for client in range(fed_args.num_clients):
        ...
        if evaluation:
            eval_results = trainer.evaluate()  # This will compute metrics based on eval_dataset
        ...
        for j in range(len(ciphertext)):  
            filename = f"ciphertexts/index/ciphertext_{client*len(ciphertext)+j}.txt"
    ...
    for client in range(fed_args.num_clients):
        ...
        local_sparse_list[client] = reconstruct_gradients_sparse(local_sparse_list[client], selected_indices, total_param) #reshape back to original size
        
        #recostruct for  comparison 
        encrypted_dict_list[client] = encrypt_gradient_for_aggregation(local_dict_list[client],selected_indices) 
           
        for j in range(len( encrypted_dict_list[client])): 
            filename = f"ciphertexts/updates/ciphertext_{client*len( encrypted_dict_list[client] )+j}.txt"
            if not SerializeToFile(filename, encrypted_dict_list[client][j], BINARY):
                print(f"Error writing ciphertext {client*len( encrypted_dict_list[client] )+j}")
                continue
    temp_time /= fed_args.sample_clients
    client_time += temp_time
    ...
    start_time = time.perf_counter()
    encrypted_aggregates, global_auxiliary = global_aggregate_encrypted( #need to return sample data
        fed_args, global_dict, sample_num_list, \
        clients_this_round, round_idx, global_encrypted_list, encrypted_dict_list, cc,keys.publicKey, sparsek,proxy_dict=proxy_dict, \
    ...
    print(f"FUSE-SPA Time: server time {server_time}s , client time {client_time}")
